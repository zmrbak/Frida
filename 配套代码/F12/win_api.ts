export class WinApi {
    ...
    private static address_AttachThreadInput: NativePointerValue | null;
    static AttachThreadInput(idAttach: number, idAttachTo: number, fAttach: number): number {
        ...
        //     [in] BOOL  fAttach
        // );
        if (this.address_AttachThreadInput == null) {
            this.address_AttachThreadInput = Module.findExportByName("User32.dll", "AttachThreadInput");
        }
        return new NativeFunction(this.address_AttachThreadInput!, "int", ["int", "int", "int"])(idAttach, idAttachTo, fAttach);
    }
}